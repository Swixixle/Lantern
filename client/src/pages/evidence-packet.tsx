...
export default function EvidencePacket() {
  ...
  const handleExportPdf = async () => {
    ...
    try {
      const pdfContent = generatePdfContent(packet);
      const blob = new Blob([pdfContent], { type: "text/plain" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `lantern-packet-${packet.packet_id}.pdf`;
      a.click();
      URL.revokeObjectURL(url);
    } finally {
      setExporting(false);
  ...
  const generatePdfContent = (p: EvidencePacketData): string => {
    const lines: string[] = [];
    ...
    lines.push("CLAIM");
    lines.push("-----");
    lines.push(`ID: ${p.claim.id}`);
    lines.push(`Classification: ${p.claim.classification}`);
    lines.push(`Confidence: ${(p.claim.confidence * 100).toFixed(0)}%`);
    lines.push(`Text: ${p.claim.text}`);
    lines.push("");
    lines.push("ANCHORS");
    lines.push("-------");
    
    p.anchors.forEach((anchor, idx) => {
      lines.push("");
      lines.push(`[${idx + 1}] Anchor ID: ${anchor.id}`);
      lines.push(`    Source: ${anchor.source_document}`);
      lines.push(`    Page: ${anchor.page_ref}`);
      if (anchor.section_ref) {
        lines.push(`    Section: ${anchor.section_ref}`);
      }
      lines.push(`    Date: ${anchor.timeline_date}`);
      lines.push(`    Quote:`);
      lines.push(`    "${anchor.quote}"`);
    });
    
    lines.push("");
    lines.push("---");
    lines.push("Generated by Lantern - Claim Governance Interface");
    lines.push("No inferred intent. No rankings. No synthesis. No conclusions.");
